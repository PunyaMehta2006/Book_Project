...
export default function Home({ books, loading, error, onFilter }) {
  ...
  const handleExchangeToggle = () => {
    const next = !exchangeOnly;
    setExchangeOnly(next);
    onFilter({
      genre: activeGenre !== "All" ? activeGenre : "",
      search: searchQuery,
      exchange: next,
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-cream-dark to-cream pt-12 pb-12 sm:pt-16 sm:pb-16 lg:pt-20 lg:pb-20">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="max-w-xl mx-auto mb-12 sm:mb-14">
            ...
            {/* Exchange Toggle Filter */}
            <div className="mt-5 flex items-center justify-center gap-3">
              <button
                role="switch"
                aria-checked={exchangeOnly}
                onClick={handleExchangeToggle}
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-sage/40 ${
                  exchangeOnly ? "bg-sage" : "bg-warm-border"
                }`}
              >
                <span
                  className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-md ring-0 transition-transform duration-200 ease-in-out ${
                    exchangeOnly ? "translate-x-5" : "translate-x-0.5"
                  }`}
                />
              </button>
              <span className="font-body text-sm text-charcoal font-medium">
                Show books open to exchange only
              </span>
            </div>
          </div>
        </div>
          ...
          <main className="flex-1 min-w-0">
            ...
            {error && (
              <div className="text-center py-16">
                ...
                <p className="font-body text-charcoal-muted">{error}</p>
              </div>
            )}
            ...
            {loading && !error && (
              <div className="text-center py-16">
                <div className="inline-block w-8 h-8 border-4 border-terracotta/30 border-t-terracotta rounded-full animate-spin mb-4"></div>
                <p className="font-body text-charcoal-muted">Loading books...</p>
              </div>
            )}
